...
# Script to run the agent worker as a cron job
...
run_agent_worker() {
    ...
    python data/workers/cron_service/agent_worker.py >> $LOG_FILE 2>&1
    echo -e "${GREEN}Agent worker job completed at $(date)${NC}" | tee -a $LOG_FILE
}

# Function to handle script termination
cleanup() {
    echo -e "${YELLOW}Shutting down agent worker...${NC}"
    exit 0
}
...
echo -e "${GREEN}Starting agent worker job...${NC}"
run_agent_worker
